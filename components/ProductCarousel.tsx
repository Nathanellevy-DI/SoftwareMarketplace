...
export default function ProductCarousel({ images = [], altText = 'Artwork Image' }: { images: string[], altText?: string }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Main Image Viewer */}
      <div className="relative aspect-[3/4] bg-[#EDEDE9]/40 border border-[#2b2522]/10 overflow-hidden shadow-md group rounded-xl">
        <img
      ...
      {images.length > 1 && (
        <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
          {images.map((img, idx) => (
            ...
            >
              <img src={img} alt={`Mockup ${idx + 1}`} className="w-full h-full object-contain p-2 bg-[#EDEDE9]/30" />
            </button>
          ))}
        </div>
      )}
